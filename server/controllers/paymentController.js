...
export const buySubscription = catchAsyncError(async (req, res, next) => {
  ...
  const plan_id = process.env.PLAN_ID || "plan_LqE7LABKjlDxiC";
...
export const paymentVerification = catchAsyncError(async (req, res, next) => {
  const { razorpay_signature, razorpay_payment_id, razorpay_subsription_id } =
    req.body;
  const user = await User.findById(req.user._id);
  ...
  await Payment.create({
    razorpay_signature,
    razorpay_payment_id,
    razorpay_subsription_id,
  });
  user.subscription.status = "active";

  await user.save();
  res.redirect(
    `${proceess.env.FRONTED_URL}/paymentsuccess?reference=${razorpay_payment_id}`
  );
});
...
export const cancelSubscription = catchAsyncError(async (req, res, next) => {
  ...
  const payment = await Payment.findById({
    razorpay_subsription_id: subscriptionId,
  });
  ...
  res.status(200).json({
    ...
    message: refund
      ...
      : "Subscription cancelled,No refund will be given as subscription was cancelled after 7 days",
  });
});
