...
const Header = ({isAuthenticated=true,user}) => {
  ...
  return (
    ...
    <>
        <ColorModeSwitch/>
    <Button zIndex={"overlay"} onClick={onOpen} position={'fixed'} colorScheme={"yellow"} rounded={'full'} top
    ={'4'} left={"4"}>
    <RiMenu5Fill/>
    </Button>
    ...
    <DrawerOverlay/>
   <DrawerContent>
   <DrawerHeader fontWeight={'600'} fontFamily={'sans-serif'} fontSize={'3xl'} textAlign={"center"} >Course Bundler</DrawerHeader>
   <DrawerBody >

    <VStack spacing={['2','5']}  alignItems={"flex-start"} >
    <Option onClose={onClose} url="/" title="Home" />
    <Option onClose={onClose} url="/courses" title="Browse All Courses" />
    <Option onClose={onClose} url="/request" title="Request A Course" />
    <Option onClose={onClose} url="/contact" title="Contact" />
    <Option onClose={onClose} url="/about" title="About" />

    
    <HStack position='absolute' bottom={"2rem"} left={"2rem"} width={"80%"} justifyContent={"space-evenly"}>
    {isAuthenticated?(<>
    <VStack>
        <HStack> <Link to="/profile">
